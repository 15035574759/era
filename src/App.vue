<template>
  <div id="app">
    <div class="upcoming" v-show="upcoming"><div>UPCOMING</div></div>
    ...
    <!-- <div :class="[navBarFixed == true ? '' : '', 'header']"> -->
...
<!--            <i>{{contractAddress.substring(0,8)}}...</i>-->
...
<!--          </div>-->
    ...
    <el-drawer :visible.sync="drawer" :size="size" :with-header="false">
      ...
      <div class="itemList">
        <img :src="item.img" alt v-for="(item, key) in navImg" :key="key" @click="toSocial(item.link)"/>
      </div>
    </el-drawer>
    <linkPackage
    ...
    <div class="popUpOuter">
      ...
      <div v-for="item  in popUpList" :key="item.id" class="outerItem">
...
import linkPackage from "./components/linkPackage.vue";

import { connectInfo, connect , connectInfoDetail} from "@/wallet/connectMetaMask";
import { mapGetters, mapState } from 'vuex'
import { userNtfSell , userNtfSellApprove } from '@/wallet/trade.js'
...
export default {
  ...
  data() {
    return {
      upcoming:false,
      contractAddress:"0x15b0eE096Ea786160390d5cD7816801E5fed871d",
  ...
  watch: {
    pendingOrderAmount:{ // 交易中数量
      handler(val){
        if(val){
          this.$store.dispatch('showPendingPopUp')
        }
      }
    },
    $route(to, from) {
  ...
  methods: {
    ...
    switchLang(){
      ...
      // userNtfSellApprove('1003' , CONFIG.nftMarket)
      // .then(()=>{
        // setTimeout(()=>{
    ...
    clickLanguageDropdown(command) {
      console.log(command);
        if(command) {
            ...
            this.$i18n.locale = command //切换字典文件
        }
    ...
    activeNavabs(key,href) {
      if(href){
        this.drawer = false
        return window.open(href , '_blank')
      }
      let item = this.navItems[key];
      // if (item.activeIndex==="/market"||item.activeIndex=="/FARMS"||item.activeIndex==="/blindbox"){
      if (item.activeIndex==="/market"){
        this.upcoming = true
        var _this = this
        setTimeout(function (){
          _this.upcoming = false
        },2000)
        return
      }
      this.$router.push({
...
.LPcolor.common_btn_disabled::after{
  background-color: rgba(38, 53, 44 , .7);
}
.header {
  width: 100%;
  z-index: 999;
...
.connect-wallet {
  background: url(./assets/img/package-v2.png) no-repeat center;
...
.connect-wallet-enter {
  background: url(./assets/img/packageenter-v2.png) no-repeat center;
  background-size: contain;
  width: 150px;
  line-height: 30px;
  height: 30px;
...
.my-nft {
  background: url(./assets/img/mypackage.png) no-repeat center;
  background-size: contain;
  width: 100px;
  line-height: 30px;
  height: 30px;
